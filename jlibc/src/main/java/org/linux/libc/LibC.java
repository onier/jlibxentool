...
public interface LibC extends Library {
    ...
    LibC INSTANCE = (LibC) Native.loadLibrary("c",
            LibC.class);
...
//    
    /**
     * Gets the value of the C variable "errno". Only valid after certain system
     * calls, and only if the system call failed in some way.
     *
...
//    @GlobalVar 
     ...
     * @return If successful, returns a non-negative integer, termed a file
     * descriptor. Returns -1 on failure, and sets errno to indicate the error.
     */
    public int open(String name, int oflag, int mode);
     ...
     * descriptor. Returns -1 on failure, and sets errno to indicate the error.
     */
    public int open(String name, int oflag);
     ...
     * a value of -1 is returned and the global integer variable errno is set to
     * indicate the error.
     */
     ...
     * beginning of the file. If error, -1 is returned and errno is set to
     * indicate the error.
     */
     ...
     * @return the number of bytes actually read is returned. Upon reading
     * end-of-file, zero is returned. If error, a -1 is returned and the global
     * variable errno is set to indicate the error
     */
    public int read(int fd, byte[] buf, int nbyte);
     ...
     * @param nbyte number of bytes to read
     * @return the number of bytes which were written is returned. If error, -1
     * is returned and the global variable errno is set to indicate the error.
     */
    public int write(int fd, byte[] buf, int nbyte);

    /**
     * C struct stat // struct stat { // dev_t	st_dev;	/* [XSI] ID of device
     * containing file 4 0 // ino_t	st_ino;	/* [XSI] File serial number 4 4 //
     * mode_t	st_mode;	/* [XSI] Mode of file (see below) 2 8 // nlink_t
     * st_nlink;	/* [XSI] Number of hard links 2 10 // uid_t	st_uid;	/* [XSI]
     * User ID of the file 4 12 // gid_t	st_gid;	/* [XSI] Group ID of the file 4
     * 16 // dev_t	st_rdev;	/* [XSI] Device ID 4 20 // time_t	st_atime;	/* [XSI]
     * Time of last access 4 24 // long	st_atimensec;	/* nsec of last access 4
     * 28 // time_t	st_mtime;	/* [XSI] Last data modification time 4 32 // long
     * st_mtimensec;	/* last data modification nsec 4 36 // time_t	st_ctime;	/*
     * [XSI] Time of last status change 4 40 // long	st_ctimensec;	/* nsec of
     * last status change 4 44 // off_t	st_size;	/* [XSI] file size, in bytes 8
     * 48 // blkcnt_t	st_blocks;	/* [XSI] blocks allocated for file 8 //
     * blksize_t	st_blksize;	/* [XSI] optimal blocksize for I/O 4 // __uint32_t
     * st_flags;	/* user defined flags for file 4 // __uint32_t	st_gen;	/* file
     * generation number 4 // __int32_t	st_lspare;	/* RESERVED: DO NOT USE! 4 //
     * __int64_t	st_qspare[2];	/* RESERVED: DO NOT USE! 16 // };
     */
    public static class stat extends Structure {

//        public final static int EPERM = IMPORT;
        /**
         * mode_t
         */
        public int st_mode;
        ...
        protected List getFieldOrder() {
            return Arrays.asList("st_mode", "st_mtime", "st_size");
        }
    ...
    /**
     * Perform IO control operation <code>request</code> on device
     * <code>fd</code>.
     *
     * @param fd an open file descriptor
     * @param request am encded value containing the requested operation and the
     * arguments
     * @param i1
     * @return -1 on error
     */
    public int ioctl(int fd, int request, int i1);

    /**
     * Perorm IO control operation <code>request</code> on device
     * <code>fd</code>.
     *
     * @param fd an open file descriptor
     * @param request am encded value containing the requested operation and the
     ...
     * @return -1 on error
     */
    public int ioctl(int fd, int request, int i1, int i2);
     ...
     * @return -1 on error
     */
    public int ioctl(int fd, int request, int i1, int i2, int i3);
